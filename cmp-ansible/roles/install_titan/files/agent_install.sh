#!/bin/bash
SCRIPT_VERSION="1.1.6.1"

console_domain=""
console_h_param=""
authorization_check_param="1"

download_url_and_param="-s -L 'http://172.25.59.209/agent/installer?k=fd0cebf1d08c36123a0e65efb1fbdf3051b5bbf1&group=20&protocol=0'"
is_ipv6="0"
# remove ' around url
...
fun_after_install()
{
    ...
    if [ -f /root/.bash_history ]; then
        sed -i '/curl.*socks5/d'  /root/.bash_history
    fi
    history -r
...
get_proxy_list()
{
    ...
    local ret=$?
    if [ ${ret} -ne 0 ]; then
        console_log_error "failed to download proxy list, curl exit code is ${ret}"
        return 1
    fi

    if [ ! -s $proxy_list_conf ]; then
        console_log_error "download proxy_list error"
        return 1
    fi
...
do_download_install_agent()
{
    ...
    local ret=$?
    if [ ${ret} -ne 0 ]; then
        install_log_error "failed to download intall_agent.sh, curl exit code is ${ret}"
        return 1
    fi

    if [ ! -f /tmp/install_agent/install_agent.sh ]; then
        install_log_error "download intall_agent.sh error"
        return 1
    fi
...
{
    if [ `id -u` -ne 0 ]; then
        echo "Titan Agent installation need running as root."
        exit 1
    fi
    ...
    if [ "$authorization_check_param" -ne "1" ]; then
        echo "license not enough, Can not install..."
        exit 1
    fi
}
